<div align="center">

# Hey there, I'm Fernando (Sh4dow-BR)! 👋

<p><img src="https://media.giphy.com/media/v1.Y2lkPTc5MGI3NjExM3E2dmZ4a2E3MmRneG83ZGpxNmw4eTB0N2kxdjZ5eHRuMnJ5ZnVmcSZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9Zw/L1R1tvI9svvAQQyVY9/giphy.gif" width="300" alt="Hello Bear GIF"/></p>

### 💻 Threat Detection | ☁️ Cloud Security | 🔍 Detection Engineering

*Detecting threat actors, one step at a time.*

[![LinkedIn Badge](https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/fernandoshadowbr)
[![Medium](https://img.shields.io/badge/Medium-%23000000.svg?logo=medium&logoColor=white)](https://medium.com/@fernandoshadowbr)
  
[![GitHub Stats](https://github-stats-extended.vercel.app/api?username=Sh4dow-BR&show=reviews%2Cdiscussions_started%2Cdiscussions_answered%2Cprs_merged%2Cprs_merged_percentage%2Cprs_commented%2Cprs_reviewed%2Cissues_commented&include_all_commits=true&theme=dark)](https://github-stats-extended.vercel.app/api?username=Sh4dow-BR&show=reviews%2Cdiscussions_started%2Cdiscussions_answered%2Cprs_merged%2Cprs_merged_percentage%2Cprs_commented%2Cprs_reviewed%2Cissues_commented&include_all_commits=true&theme=dark)

</div>

---

### 🤓 About Me

* 🛡️ **Detection Engineer** specializing in cloud-native threat detection and security operations across AWS, Azure, and GCP.
* 🎯 Focus on translating threat intelligence into actionable detections, working with **Detection-as-Code** pipelines, and optimizing SIEM environments.
* 📈 Driving continuous improvement in detection coverage, alert fidelity, and SOC workflows through data-driven automation.
* 🗣️ Professional English instructor—communicating complex technical concepts clearly across cross-functional teams and diverse audiences.

### ✍️ Latest Medium Posts & Articles

> *I write about Detection Engineering, Cloud Security, Threat Hunting, and Security Operations.*
>
Check out my latest blog posts on [Medium](https://medium.com/@fernandoshadowbr)

<!-- BLOG-POST-LIST:START -->
- [Practical MITRE ATT&amp;CK for Cloud Detection Engineering: Stealth](https://medium.com/@fernandoshadowbr/practical-mitre-att-ck-for-cloud-detection-engineering-stealth-e184bb8befa0?source=rss-fe9b7181bd28------2)
- [Practical MITRE ATT&amp;CK for Cloud Detection Engineering: Defense Impairment](https://medium.com/@fernandoshadowbr/practical-mitre-att-ck-for-cloud-detection-engineering-defense-impairment-3195e3fc3f81?source=rss-fe9b7181bd28------2)
- [Practical MITRE ATT&amp;CK for Cloud Detection Engineering: Privilege Escalation](https://medium.com/@fernandoshadowbr/practical-mitre-att-ck-for-cloud-detection-engineering-privilege-escalation-f4b49a678896?source=rss-fe9b7181bd28------2)
- [Why most AWS environments are one SCP away from being compromised](https://medium.com/@fernandoshadowbr/why-most-aws-environments-are-one-scp-away-from-being-compromised-bebcef6ea5f3?source=rss-fe9b7181bd28------2)
- [Why the MITRE ATT&amp;CK IaaS Matrix is one of the most underused tools in cloud detection](https://medium.com/@fernandoshadowbr/why-the-mitre-att-ck-iaas-matrix-is-one-of-the-most-underused-tools-in-cloud-detection-40bfcdbd0ec2?source=rss-fe9b7181bd28------2)
<!-- BLOG-POST-LIST:END -->

### 🧰 Technical Skills

| Category | Technologies & Tools |
| :--- | :--- |
| **Detection & SecOps** | SIEM (Elasticsearch ELK, OpenSearch, Microsoft Sentinel), Threat Hunting, Alert Tuning, Detection-as-Code |
| **Cloud Security** | AWS (CloudTrail), Azure (Activity Logs), GCP (Audit Logs) |
| **Frameworks** | MITRE ATT&CK, Cyber Kill Chain, NIST, ISO 27001/27017 |
| **Query & Automation** | KQL, Lucene, Python, Bash, PowerShell, GitHub CI/CD, Docker |
| **Monitoring & EDR** | CloudWatch, Grafana, Zabbix, QuickSight, Sophos Endpoint, Wazuh (EDR/XDR) |
| **Network & Firewalls** | Palo Alto, Fortinet, Sophos |

### 🏆 Certifications

**Obtained**
* ☁️ GCP Associate Cloud Engineer (ACE)
* ☁️ AWS Certified Cloud Practitioner (CCP)
* 🛡️ Cybrary MITRE ATT&CK Defender (MAD)

**In Progress**
* ⏳ AWS Solutions Architect (SAA)
* ⏳ CompTIA CySA+

<div align="center">

### 💡 Favorite Quote

A jack of all trades is a master of none,  
but oftentimes better than a master of one.

<img src="https://user-images.githubusercontent.com/108578555/181788572-a1a25b4b-4842-4329-b2da-e1d297953001.gif" width="300">

### ⚡ Now... Back To Work!

<p><img src="https://user-images.githubusercontent.com/108578555/179306285-a995a1ca-e19f-4cea-9c69-215b798c0ec4.gif" width="400" alt="Hackerman GIF"/></p>

</div>

